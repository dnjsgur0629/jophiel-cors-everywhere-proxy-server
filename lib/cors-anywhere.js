...
let help_text = {}
const showUsage = (help_file, headers, response) => {
...
const proxyRequest = (req, res, proxy) => {
  ...
  if (proxyThroughUrl) {
    ...
    onsole.log('location.href', location.href)
 ...
 */
const onProxyResponse = (proxy, proxyReq, proxyRes, req, res) => {
...
const getHandler = (options, proxy) => {
  const corsAnywhere = {
  ...
  if (corsAnywhere.requireHeader) {
    if (typeof corsAnywhere.requireHeader === 'string') {
      ...
      corsAnywhere.requireHeader = corsAnywhere.requireHeader.map((headerName) => {
