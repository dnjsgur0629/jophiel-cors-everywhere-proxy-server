...
import { createRateLimitChecker } from '../lib/rate-limit.js'

const assertNotLimited = (rateLimitReturnValue) => {
  if (rateLimitReturnValue) {
    assert.fail('Expected no limit, but got ' + rateLimitReturnValue)
  }
}

const assertLimited = (rateLimitReturnValue, limit, period) => {
  let msg
  if (period === 1) {
...
describe('Rate limit', () => {
  ...
  it('one per minute', () => {
    const checkRateLimit = createRateLimitChecker('1 1')
    assertNotLimited(checkRateLimit('http://example.com'))
  ...
  it('unlimited domains, string', () => {
    const checkRateLimit = createRateLimitChecker('1 2 example.com')
    assertNotLimited(checkRateLimit('http://example.com'))
    assertNotLimited(checkRateLimit('http://example.com'))
  ...
  it('multiple domains, string', () => {
    const checkRateLimit = createRateLimitChecker('1 2 a b  cc ')
  ...
  it('multiple domains, string and RegExp', () => {
    const checkRateLimit = createRateLimitChecker('1 2 a /b/')
    assertNotLimited(checkRateLimit('http://a'))
    assertNotLimited(checkRateLimit('http://a'))
    assertNotLimited(checkRateLimit('http://b'))
    assertNotLimited(checkRateLimit('http://b'))
  ...
  it('wildcard subdomains', () => {
    const checkRateLimit = createRateLimitChecker('0 1 /(.*\\.)?example\\.com/')
    assertNotLimited(checkRateLimit('http://example.com'))
    assertNotLimited(checkRateLimit('http://www.example.com'))
  ...
  it('wildcard ports', () => {
    const checkRateLimit = createRateLimitChecker('0 1 /example\\.com(:\\d{1,5})?/')
    assertNotLimited(checkRateLimit('http://example.com'))
    assertNotLimited(checkRateLimit('http://example.com:1234'))
  })
